...
function M.parse(arg)
   ...
   cmd:option('-datasetDir',       '/home/yumin/dataset/MPII',         'Path to dataset')
   cmd:option('-dataset',    'mpii', 'Options: mpii')
   ...
   cmd:option('-outputRes',       16,     'Output resolution')
   ...
   cmd:option('-depth',        152,       'ResNet depth: 18 | 34 | 50 | 101 | ...', 'number')
   ...
   elseif opt.dataset == 'mpii' then
      opt.shorcutType = opt.shortcutType == '' and 'B' or opt.shortcutType
      opt.nEpochs = opt.nEpochs == 0 and 90 or opt.nEpochs
   else
